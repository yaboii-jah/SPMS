...
export function ViewSpms () {
    ...
    useEffect(() => {
        function filterData () {
            ...
            data[1].performance.forEach(perf => {
            ...
            ratingsCopy = data[1].ratings

            setRatings(ratingsCopy[0])
            setUser(data[0][0])
            setStrat(stratCopy)
            setCore(coreCopy)
                ...
                <div className='container'> 
                    ...
                    <div className="ipcr-form">    
                        <div className="view-ipcr-container">
                            <h4> {user.role === 'IPCR' ? 'INDIVIDUAL' : user.role  === 'DPCR' ? 'DIVISION' : 'OFFICE'} PERFORMANCE COMMITMENT AND REVIEW FORM ({user.role})</h4>
                    ...
                    <div className="view-signature-section">
                        ...
                        <div className="view-label">{user.role === 'IPCR' ? 'Employee' : user.role === 'DPCR' ? 'OIC/Division Chief/Section Head' : 'Director'}</div>
                    ...
                    <table className="view-table">
                        ...
                        <tr>
                            <td style={{textAlign: "center"}}>{user.role === 'IPCR' ? (user.supervisor_division_chief) : user.role === 'DPCR' ? user.office_director : user.commissioner}</td>
                            <td rowSpan={2} style={{textAlign: "center"}}>{user.created_at ? formatDate(user.created_at) : ""}</td>
                            <td style={{textAlign: "center"}}>{user.role === 'IPCR' ? user.office_director : user.role === 'DPCR' ? user.commissioner : user.chairperson}</td>
                            <td rowSpan={2} style={{textAlign: "center"}}>{user.created_at ? formatDate(user.created_at) : ""}</td>
                        </tr>
                        <tr>
                            <td style={{backgroundColor: "rgb(224, 224, 224)", textAlign: "center", fontWeight: "bold"}}>{user.role === 'IPCR' ? 'Immediate Supervisor/Division Chief' : user.role === 'DPCR' ? 'Director' : 'Commissioner In-charge'}</td>
                            <td style={{backgroundColor: "rgb(224, 224, 224)", textAlign: "center", fontWeight: "bold"}}>{user.role === 'IPCR' ? 'Head Of Office/Director' : user.role === 'DPCR' ? 'Commissioner In-charge' : 'Chairperson'}</td>
                        </tr>
                        <tr>
                    ...
                    <table className="view-table">
                        ...
                        <tr>
                            <td colSpan={2}></td>
                            <td colSpan={user.role === 'IPCR' ? 4 : 6} style={{backgroundColor: "rgb(221, 221, 221)", textAlign: "center"}}>Average Rating</td>
